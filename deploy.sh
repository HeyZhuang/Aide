...
echo "📝 创建生产环境配置文件..."
# VITE_JAAZ_BASE_API_URL 设置为本地地址，通过前端服务器代理（使用相对路径）
# 这样便于二次开发新产品，所有认证请求都通过本地后端处理
# VITE_BACKEND_URL 用于本地后端服务
cat > .env.production << EOF
VITE_BACKEND_URL=http://127.0.0.1:${BACKEND_PORT}
# VITE_JAAZ_BASE_API_URL 设置为空字符串，使用相对路径通过前端服务器代理到本地后端
VITE_JAAZ_BASE_API_URL=
EOF
echo "✅ 环境配置文件已创建/更新（使用本地认证服务）"
